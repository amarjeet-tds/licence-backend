...
from utils import jwt_required, is_allowed_hit, log_request , is_plan_active
...
def hit_api():
    ...
    try:
        payload = request.jwt_payload
        res , status =  {"message": "API Hitted successfully"}, 200
        req_status = 'Success'
        return  Response(json.dumps(res), status=status, mimetype='application/json')
    except Exception as e:
        req_status = 'Error'
...
def user_dashboard():
    try:
        res , status =  user_dashboard_fn(db_engine)
        return  Response(json.dumps(res), status=status, mimetype='application/json')
    except Exception as e:
        print(e)
        return Response(json.dumps({"message":"Internal Server Error"}), status=500, mimetype='application/json')
